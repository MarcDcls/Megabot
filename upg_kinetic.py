...
def init(n=100, passenger=True, passenger_weight=80):
    ...
    set_O([0., 0., 580.])
    set_omega([0., 0., 0.])
    com_rel = center_of_mass(V0, passenger_weight=passenger_weight)
...
def move_abs_legs_and_com(traj_leg, leg_id, traj_com, reg_val=0.01, const_omega=True, max_omega=10, passenger_weight=80.0):
    V = get_verins_12()
    O = get_O()
    Omega = get_omega()
    LV = [V]
    ...
    traj_legs = gen_traj_all_legs(traj_leg, leg_id, X0)
    ...
    for i in range(1, len(traj_legs)):
        ...
        set_com(com)
        dX = np.concatenate((traj_legs[i] - X0, traj_com[i] - com[0:2]))
...
def M_letter_one_leg(n, amp, pos_ini):
    X = np.full(6 * n, pos_ini[0])
    Y = np.full(2 * n, pos_ini[1])
    Y = np.append(Y, np.linspace(pos_ini[1], pos_ini[1] - amp, n))
    Y = np.append(Y, np.linspace(pos_ini[1] - amp, pos_ini[1] - 2 * amp, n))
    Y = np.append(Y, np.full(2 * n, pos_ini[1] - 2 * amp))
    Z = np.linspace(pos_ini[2], pos_ini[2] + 2.5 * amp, 2 * n)
    Z = np.append(Z, np.linspace(pos_ini[2] + 2.5 * amp, pos_ini[2] + 1.5 * amp, n))
    Z = np.append(Z, np.linspace(pos_ini[2] + 1.5 * amp, pos_ini[2] + 2.5 * amp, n))
    Z = np.append(Z, np.linspace(pos_ini[2] + 2.5 * amp, pos_ini[2], 2 * n))
